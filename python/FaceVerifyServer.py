...
import retrieve as retrieveImg
import sys
...
countRec = int(retrieveImg.numberOfRowsInDB())
...
if len(image_1)<1 :
    ...
    image_1 = #path of image from user
    subscription_key, face_api_url, face_api_url_verify  = cnfg.config()
'''
for i in range(1, countRec+1):
    '''
    #image_2 = input("Enter Image 2 Path or url ")
    # image_2 = db-image.jpg
    if len(image_2)<1 :
        #print("Image Path not Entered using Sample Image")
        '''
    image_2, celebrityName = retrieveImg.getImageFromDB(i)
    faces_1 = image_processing(image_1, subscription_key, face_api_url)
    faces_2 = image_processing(image_2, subscription_key, face_api_url)
    ...
    if result['isIdentical'] == True:
        print("Celebrity Name: ", celebrityName)
        print("CONFIDENCE: " , result['confidence'])
        print("END of Python Program")
        break

    print("No Match Found!!!")
    retrieveImg.DBImgDelete(celebrityName)